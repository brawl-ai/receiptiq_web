...
export async function fetchProjects(): Promise<PaginatedResponse<ProjectResponse> | null> {
  try {
    ...
    const response = await axios.get<PaginatedResponse<ProjectResponse>>(
      `${process.env.BACKEND_API_BASE}/api/v1/projects`,
      {
        headers: {
...
export async function getCurrentUser(): Promise<User | null> {
  ...
  try {
    const res = await axios.get<User>(
      `${process.env.BACKEND_API_BASE}/api/v1/auth/me`,
      {
        headers: {
          Cookie: `access_token=${access_token}`,
          "Content-Type": "application/json",
...
export async function getProject(id: string): Promise<ProjectResponse | null> {
  try {
    ...
    const response = await axios.get<ProjectResponse>(
      `${process.env.BACKEND_API_BASE}/api/v1/projects/${id}`,
      {
        headers: {
          Cookie: `access_token=${access_token}`,
          "Content-Type": "application/json",
